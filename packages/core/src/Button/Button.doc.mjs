...
export const docs = {
  ...
  theming: {
    targets: [
      {
        ...
        visualProps: ['size', 'variant', 'elevation', 'content'],
      },
      {className: 'astryx-button-icon', visualProps: ['size']},
      {className: 'astryx-button-label', visualProps: ['size']},
      {className: 'astryx-button-end-content', visualProps: ['size']},
    ],
    vars: [
      {
        name: '--_button-radius',
    ...
    derived: [{property: 'borderRadius', vars: ['--_button-radius']}],
  },
};
  ...
  props: [
    {
      name: 'label',
      type: 'string',
      description: '无障碍标签；纯图标按钮时用作 aria-label。',
      required: true,
    },
    {
      name: 'variant',
    ...
    },
    {
      ...
      type: "'none' | 'low' | 'med' | 'high'",
      description:
        '浮动按钮（如 FAB）的静止阴影深度。`none` 为默认扁平按钮；在 ButtonGroup 内忽略。',
      default: "'none'",
    },
    {
      name: 'type',
    ...
    },
    {
      ...
      type: 'boolean',
      description:
        '禁用按钮。存在工具提示时，使用 aria-disabled 代替原生 disabled 以保持可聚焦。',
      default: 'false',
    },
    {
      name: 'icon',
    ...
    },
    {
      ...
      type: 'SizeValue',
      description:
        "按钮宽度。数字按像素处理，字符串按原样使用（如 '100%' 表示全宽按钮）。默认按内容自适应宽度。",
    },
    {
      name: 'children',
      type: 'ReactNode',
      description:
        '可选的可见内容覆盖；label 仍然是必需的（用于无障碍名称）。大多数情况使用 <Button label="Save" />。',
    },
    {
      name: 'endContent',
      type: 'ReactElement<IconProps> | ReactElement<BadgeProps>',
    ...
    {name: 'tooltip', type: 'string', description: '悬停时显示的提示文本。'},
    {
      name: 'onClick',
      type: '(e: MouseEvent) => void',
      description: '标准点击处理函数（从 ButtonHTMLAttributes 透传）。',
    },
    {
      name: 'clickAction',
      type: '(e: MouseEvent) => void | Promise<void>',
    ...
    bestPractices: [
      {
        guidance: true,
        description:
          'Primary for the single most important action. Secondary or ghost for the rest.',
      },
      {
        guidance: true,
        description:
          'Labels that describe the action: "Save changes" not "OK" or "Click here".',
      },
      {
        guidance: true,
        description:
          'Show loading state for async actions so the user knows it is working.',
      },
      {
        guidance: true,
        description:
          'Icon-only buttons need a label for screen readers and a tooltip for sighted users.',
      },
      {
        guidance: true,
        description:
          'For dedicated icon-only buttons, use IconButton from @astryxdesign/core/IconButton. Separate component, not exported from @astryxdesign/core/Button.',
      },
      {
        guidance: false,
        description: 'Multiple primary buttons in one view; dilutes hierarchy.',
      },
      {
        guidance: false,
        description:
          'Destructive without confirmation for irreversible actions.',
      },
      {
        guidance: false,
        description:
          'Button for navigation; use a link if it only takes the user to another page.',
      },
    ],
  },
  propDescriptions: {
    label:
      'accessible label; visible text by default, aria-label when isIconOnly',
    variant: 'visual style variant',
    size: 'size variant',
    elevation:
      'resting shadow depth for floating buttons/FABs: none|low|med|high; ignored inside ButtonGroup',
    ...
    isIconOnly:
      'when true, renders square icon-only button; label becomes aria-label',
    width:
      "Width of button. Numbers=pixels, strings=as-is (e.g. '100%' for full-width).",
    children:
      'optional visible override; label is still required for a11y. Prefer <Button label="Save" /> over using children',
    endContent:
      'trailing icon/badge after label; ignored when isIconOnly; color inherited',
