...
export const docs = {
  ...
  props: [
    {
      name: 'variant',
      type: "'neutral' | 'primary' | 'secondary' | 'critical' | 'neutral-subtle' | 'primary-subtle' | 'secondary-subtle' | 'critical-subtle' | 'info' | 'success' | 'warning' | 'error' | 'blue' | 'cyan' | 'green' | 'orange' | 'pink' | 'purple' | 'red' | 'teal' | 'yellow'",
      description:
        'Visual style variant. Minim canonical variants use neutral, primary, secondary, and critical solid or subtle appearances. Released semantic and color names remain compatibility aliases.',
      default: "'neutral'",
    },
    {
      name: 'size',
  ...
  theming: {
    targets: [
      {className: 'astryx-badge', visualProps: ['variant', 'size']},
      {className: 'astryx-badge-icon', visualProps: ['size']},
      {className: 'astryx-badge-label', visualProps: ['size']},
    ],
  },
  usage: {
    description:
  ...
  props: [
    {
      name: 'variant',
      type: "'neutral' | 'primary' | 'secondary' | 'critical' | 'neutral-subtle' | 'primary-subtle' | 'secondary-subtle' | 'critical-subtle' | 'info' | 'success' | 'warning' | 'error' | 'blue' | 'cyan' | 'green' | 'orange' | 'pink' | 'purple' | 'red' | 'teal' | 'yellow'",
      description: '视觉样式变体。Minim 规范名称与已发布的兼容别名均受支持。',
      default: "'neutral'",
    },
    {
      name: 'size',
    ...
    bestPractices: [
      {
        guidance: true,
        description:
          'Every badge steals attention. Only badge states where the user needs to act. If no follow-up is needed, use plain text.',
      },
      {
        guidance: true,
        description:
          'Use success/warning/error ONLY for system status requiring attention (Failed, Degraded, Action Required). These are visually loud: solid colored backgrounds.',
      },
      {
        guidance: true,
        description:
          'Use color variants (blue, purple, teal) for category tags that classify items: team names, content types, priority levels.',
      },
      {
        guidance: true,
        description:
          'Keep labels to one or two words. If more detail is needed, put it in surrounding text instead of the badge.',
      },
      {
        guidance: true,
        description:
      ...
      },
      {
        guidance: false,
        description:
          'Apply "success" badges to every healthy/normal item. If most rows are green "Active", none stand out. Skip the badge for the default state; only highlight exceptions that need attention.',
      },
      {
        guidance: false,
        description:
          'Use badges for metadata. Durations, counts, dates, descriptions → use Text with type="supporting" instead.',
      },
      {
        guidance: false,
        description:
          'Use status variants for non-status info. "6h window", "12 types", category names are NOT statuses.',
      },
      {
        guidance: false,
        description:
          'Repeat loud badges in every row. Common/default states should be plain text; reserve badges for the exceptional.',
      },
      {
        guidance: false,
        description:
          'Make badges clickable; they are read-only. Use a button or link for actions.',
      },
    ],
    anatomy: [
      {name: 'Icon', required: false, description: 'Optional leading icon.'},
      {
        name: 'Label',
        required: true,
        description: 'Text or number shown inside the badge.',
      },
    ],
  },
  propDescriptions: {
    variant: 'visual style variant',
    size: 'md, lg, or accessible dot size',
    label:
      'badge text content; over-wide labels clip with an ellipsis, full text in title',
