...
export const docsZh = {
  ...
  displayName: 'Side Nav Collapse Button',
  description:
    '侧边栏折叠切换按钮。放置在 SideNav 内部（自动读取上下文）或外部（向两者传入同一个受控 collapsible 配置）。默认渲染为仅图标的 ghost 按钮。',
  props: [
    {
      name: 'collapsible',
      type: '{isCollapsed: boolean, onCollapsedChange: (isCollapsed: boolean) => void}',
      description:
        '与 SideNav 相同的受控 collapsible 配置。仅在按钮渲染在侧边栏外部、无法读取折叠上下文时需要。',
    },
    {
      name: 'handleRef',
      type: 'RefObject<SideNavImperativeCollapseHandle | null>',
      description:
        '已废弃。来自 SideNav 的命令式折叠句柄；请改用 collapsible。',
    },
    {
      name: 'label',
      type: 'string',
    ...
    },
    {
      ...
      type: "'sm' | 'md' | 'lg' | 'xl'",
      description:
        "按钮尺寸。默认使用容器级联的尺寸（SideNav 页脚内为 'sm'），无容器时为 'md'。当按钮位于已设定尺寸的容器之外且需要与相邻元素保持一致时显式设置。",
    },
    {
      name: 'children',
      type: 'ReactNode',
...
export const docsDense = {
  ...
  displayName: 'Side Nav Collapse Button',
  description:
    'Toggle button for sidenav collapse. Place inside SideNav (reads context) or outside (pass the same controlled collapsible config both get). Icon-only ghost button by default.',
  propDescriptions: {
    collapsible:
      'Controlled collapsible config, same object SideNav gets. Only needed when button rendered outside sidenav.',
    handleRef: 'Deprecated. Pass collapsible instead.',
    label:
      'Custom label. Text button w/ chevron when provided, icon-only when omitted.',
